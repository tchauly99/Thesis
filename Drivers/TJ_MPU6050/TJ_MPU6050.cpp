...
static float gyro_bias[3]={-2.8, -0.3, 1.25};
static float accel_bias[3] = {0.0, 0.0, -60.0};
//3- Bias varaibles
...
void I2C_Read(uint8_t ADDR, uint8_t *i2cBif, uint8_t NofData)
{
	...
	MPUADDR = (MPU_ADDR<<1);
	i2cBuf[0] = ADDR;
	HAL_I2C_Master_Transmit(&hi2c1, MPUADDR, i2cBuf, 1, 10);
	HAL_I2C_Master_Receive(&hi2c1, MPUADDR, i2cBif, NofData, 100);
}
...
void I2C_Write8(uint8_t ADDR, uint8_t data)
{
	...
	HAL_I2C_Master_Transmit(&hi2c1, MPUADDR, i2cData, 2,100);
}
...
void MPU6050_Config(MPU_ConfigTypeDef *config)
{
	...
	//Enable Bypass mode to access Magnetometer data
//	I2C_Read(INT_PIN_CFG, &Buffer, 1);
//	Buffer |= 0x02;
...
void MPU6050_Get_Accel_Scale(ScaledData_Def *scaledDef)
{
	...
	//Accel Scale data 
	scaledDef->x = ((AccelRData.x+0.0f)*accelScalingFactor) - accel_bias[0];
	scaledDef->y = ((AccelRData.y+0.0f)*accelScalingFactor) - accel_bias[0];
	scaledDef->z = ((AccelRData.z+0.0f)*accelScalingFactor) - accel_bias[0];
}
...
void MPU6050_Get_Gyro_Scale(ScaledData_Def *scaledDef)
{
	...
	//Gyro Scale data 
	scaledDef->x = (myGyroRaw.x)*gyroScalingFactor - gyro_bias[0]; // x-Axis
	scaledDef->y = (myGyroRaw.y)*gyroScalingFactor - gyro_bias[1]; // y-Axis
	scaledDef->z = (myGyroRaw.z)*gyroScalingFactor - gyro_bias[2]; // z-Axis
}
...
void MPU6050_Bypass_En(void){
	uint8_t buffer;
	buffer |= 0x00;
	I2C_Write8(USER_CNT_REG, buffer);
	I2C_Read(INT_PIN_CFG, &Bypas_buffer, 1);
	buffer = Bypas_buffer|0x02;
	I2C_Write8(INT_PIN_CFG, buffer);
}
void MPU6050_Bypass_Dis(void){
//	I2C_Read(INT_PIN_CFG, &Buffer, 1);
//	Buffer &= ~0x02;
	I2C_Write8(INT_PIN_CFG, Bypas_buffer);
}
